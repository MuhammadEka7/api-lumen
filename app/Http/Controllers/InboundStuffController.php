...
class InboundStuffController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $result = StuffStock::where('stuff_id', $request->input('stuff_id'))->pluck('total_avaible')->first();
            $result1 = $result + $request->input('total');

            $stuffStock = StuffStock::where('stuff_id', $request->input('stuff_id'))->update(['total_avaible' => $result1]);
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $total_stock = (int)$inboundStuff->stuff->stock->total_avaible + $total_s;
    ...
    //         $avaible_min = $stock->total_avaible - $inboundStuff->total;
    //         $avaible = ($avaible_min < 0) ? 0 : $avaible_min;
    //         $defect = ($avaible_min < 0) ? $stock->total_defect + ($avaible * -1) : $stock->total_defect;
    //         $stock->update([
    //             'total_avaible' => $avaible,
    //             'total_defect' => $defect
    //         ]);
    ...
    //         // return response()->json([
    //         //     'success' => true,
    //         //     'message' => "Barang masuk berhasil dihapus dengan id $id",
    //         //     'data' => ['id' => $id]
    //         // ], 200);
    ...
    //         // return response()->json([
    //         //     'success' => false,
    //         //     'message' => "Proses gagal! Data barang masuk dengan id $id tidak ditemukan"
    //         // ], 404);
    ...
    public function destroy($id)
    {
        try {
            ...
            if ((int)$stock->total_avaible < (int)$inboundStuff->total) {
